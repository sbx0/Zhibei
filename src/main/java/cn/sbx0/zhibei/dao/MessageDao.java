...
public interface MessageDao extends PagingAndSortingRepository<Message, Integer> {
     ...
     * @return
     */
    @Query(value = "FROM Message m WHERE (m.receiveUser.id = ?1 AND m.type = 'msg') OR (m.type = 'notice')")
     ...
     * @return
     */
    @Query(value = "SELECT COUNT (m) FROM Message m WHERE (m.receiveUser.id = ?1 AND m.type = 'msg') OR m.type = 'notice'")
